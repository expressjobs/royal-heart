-- Split Discover into real-user and demo-profile paths.
--
-- Real users keep the existing full discovery filters.
-- Demo profiles use a simplified launch-safe path while demo_profiles_visible()
-- is true, without requiring reciprocal demo preference.
  ...
  FROM (
    ...
    WHERE t.id <> v_uid
      ...
      AND NOT EXISTS (
        SELECT 1
        FROM public.likes l
        WHERE l.liker_id = v_uid
          AND l.liked_id = t.id
      )
      AND (
        (
          t.is_demo_profile IS TRUE
          AND v_show_demo IS TRUE
          AND t.is_active IS TRUE
          AND t.onboarding_complete IS TRUE
          AND t.incognito IS FALSE
          AND public.pref_match(v_interested, t.gender)
        )
        OR (
          t.is_demo_profile IS NOT TRUE
