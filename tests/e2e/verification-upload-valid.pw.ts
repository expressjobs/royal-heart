...
/**
 * Happy-path coverage for the verification flow: a valid selfie is accepted by
 * the `submitVerification` server function and a `pending` verification request
 * is recorded. This suite mutates data, so it cleans up after itself.
 */
...
test.describe("verification selfie happy path", () => {
  ...
  test("accepts a valid selfie and records a pending request", async ({ request }) => {
    const res = await request.post(serverFnUrl, {
      ...
      multipart: {
        file: {
          name: "selfie.png",
          mimeType: "image/png",
          buffer: pngBuffer(VALID_PNG_500_B64), // real 500×500 PNG
        },
      },
    });
    ...
    expect(body).toContain('"request"'); // success branch carries the new request
